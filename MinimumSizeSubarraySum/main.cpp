...
{
    int len = nums.size(), start = 0, sum = 0, min_len = len + 1;
    for (int i = 0;i < len;++i)
    {
        sum += nums[i];
        if (sum >= s)
...
{
    int len = sum.size(), right = len, mid;
    while (left < right)
    {
        mid = (left + right) >> 1;
        if (sum[mid] >= target) right = mid;
        else left = mid + 1;
    }
...
{
    int len = nums.size(), start = 0, res = len + 1;
    vector<int> sum(len + 1, 0);
    for (int i = 1;i <= len;i++) sum[i] = sum[i-1] + nums[i-1];
    while (start < len)
    {
        int end = binary_search(sum, start + 1, s + sum[start]);
        if (end < 0) break;
...
int main()
{
    ...
    //    cout << minSubArrayLen(697439, nums) << endl;
